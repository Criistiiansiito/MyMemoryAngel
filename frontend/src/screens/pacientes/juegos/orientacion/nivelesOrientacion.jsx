...
const LEVELS = [
  {
    id: 'orientacion_facil',
    title: 'Nivel Básico',
    description: 'Entorno y tiempo cotidiano.',
    icon: 'circle-outline',
    color: '#EEF2FF',
    iconColor: '#6366F1',
  },
  {
    id: 'orientacion_medio',
    title: 'Nivel Intermedio',
    description: 'Relaciones temporales y espaciales.',
    icon: 'circle-half-full',
    color: '#EEF2FF',
    iconColor: '#6366F1',
  },
  {
    id: 'orientacion_dificil',
    title: 'Nivel Avanzado',
    description: 'Orientación compleja y fechas exactas.',
...
const ORIENTACION_KEYS = ['orientacion_facil', 'orientacion_medio', 'orientacion_dificil'];
...
export default function NivelesOrientacion({ onBack, onSelectDifficulty }) {
    ...
    const cargarDatos = async () => {
      try {
        ...
        const nextProgressMap = rows.reduce((acc, item) => {
          ...
          if (ORIENTACION_KEYS.includes(key)) {
            acc[key] = item;
          }
          return acc;
        }, {});
  ...
  const summary = useMemo(() => {
    const rows = ORIENTACION_KEYS.map((key) => progressMap[key]).filter(Boolean);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <View style={[styles.settingsCard, { marginTop: 8, alignItems: 'center', paddingVertical: 24 }]}>
            ...
            <Text style={[styles.sectionTitle, { marginTop: 12, textAlign: 'center' }]}>Tu progreso en Orientación</Text>
